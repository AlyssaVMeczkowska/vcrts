...
public class ControllerRequestPage extends JFrame {
    ...
    private void loadPendingRequests() {
        ...
        for (Request request : pendingRequests) {
            ...
            tableModel.addRow(new Object[]{
                    request.getRequestId(), type, request.getUserName(), timestamp
            });
        }
    ...
    private void handleAccept() {
        ...
        int confirm = JOptionPane.showConfirmDialog(this,
                ...
                "Confirm Acceptance", JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE);
    ...
    private void handleReject() {
        ...
        int requestId = (int) tableModel.getValueAt(selectedRow, 0);
        String reason = JOptionPane.showInputDialog(this, "Enter rejection reason (optional):", "Reject Request", JOptionPane.QUESTION_MESSAGE);
    ...
    private boolean processJobSubmission(Request request) {
        try {
            ...
            for (String line : lines) {
                ...
                if (parts.length == 2) jobData.put(parts[0].trim(), parts[1].trim());
            }
            Job job = new Job(
    ...
    private boolean processVehicleSubmission(Request request) {
        try {
            ...
            if (data.startsWith("SOCKET_")) data = data.split("\n", 2)[1];
            String[] lines = data.split("\n");
            Map<String, String> vehicleData = new HashMap<>();
            for (String line : lines) {
                if (line.startsWith("type:") || line.equals("---")) continue;
                String[] parts = line.split(": ", 2);
                if (parts.length == 2) vehicleData.put(parts[0].trim(), parts[1].trim());
            }
            int ownerId = Integer.parseInt(vehicleData.getOrDefault("user_id", "0"));
            Vehicle vehicle = new Vehicle(0, ownerId,
                    vehicleData.get("vehicle_make"), vehicleData.get("vehicle_model"),
                    Integer.parseInt(vehicleData.get("vehicle_year")),
                    vehicleData.get("vin"), vehicleData.get("license_plate"),
